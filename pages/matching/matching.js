    ...
    // this.countDown();
      ...
      wx.onSocketMessage(function (res) {
          ...
          if(json.status==201){
              ...
              console.log(json.data.play,that.data.play[0],that.data.play[0][0].name);
              // for(var i=0;i<arr.length;i++){
              //     if(parseInt(arr[i].uid)==parseInt(wx.getStorageSync('strWXOpenID').uid)){
              //         that.setData({
              //             user_index:i
              //         })
              //     }else{
    ...
    btnOpClick: function (e) {
        ...
        if(that.data.num==that.data.postList.length){
            if (select == 1) {
                clearInterval(m_times);
                wx.sendSocketMessage({
                    data: '{"uid":' + wx.getStorageSync('strWXOpenID').uid + ',"message":"huida_cuowu","types":"game_msg"}'
                });
                wx.sendSocketMessage({
                    data: '{"uid":' + wx.getStorageSync('strWXOpenID').uid + ',"message":"game_over","types":"game_msg"}'
                });
                ...
                clearInterval(m_times);
                wx.sendSocketMessage({
                    data: '{"uid":' + wx.getStorageSync('strWXOpenID').uid + ',"message":"huida_zhengque","types":"game_msg"}'
                });
                wx.sendSocketMessage({
                    data: '{"uid":' + wx.getStorageSync('strWXOpenID').uid + ',"message":"game_over","types":"game_msg"}'
                });
